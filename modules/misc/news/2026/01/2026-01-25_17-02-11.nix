{ config, ... }:
{
  ...
  message = ''
    The `services.syncthing.passwordFile` option has been retired, as
    ...
    The new `services.syncthing.guiCredentials` submodule will now only change
    ...
    will not need the API key reconfiguring repeatedly.
  '';
}
