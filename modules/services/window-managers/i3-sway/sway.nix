    ...
    ((optional (cfg.extraConfigEarly != "") cfg.extraConfigEarly)
      ++ (if cfg.config != null then
        ...
        [ ]) ++ (optional cfg.systemd.enable ''
          exec "systemctl --user reset-failed; ${pkgs.dbus}/bin/dbus-update-activation-environment --systemd DISPLAY WAYLAND_DISPLAY SWAYSOCK XDG_CURRENT_DESKTOP XDG_SESSION_TYPE NIXOS_OZONE_WL XCURSOR_THEME XCURSOR_SIZE; systemctl --user start sway-session.target; swaymsg -mt subscribe '[]'; systemctl --user stop sway-session.target"'')
