...
in
{
  ...
  options.wayland.windowManager.niri = {
    ...
    extraConfig = lib.mkOption {
      ...
      '';
      description = ''
        Extra configuration lines added to {file}`$XDG_CONFIG_HOME/niri/config.kdl`.

        By default, lines are appended after the generated configuration.
        To insert them before instead, use `lib.mkBefore`.
      '';
    };
  };
  ...
  config = lib.mkIf cfg.enable {
    ...
    wayland.windowManager.niri.extraConfig =
      let
        ...
        defaultConfig = ''include "${cfg.package.src}/resources/default-config.kdl"'';
        settings = lib.removeSuffix "\n" (toKDL cfg.settings);
      in
      lib.mkMerge [
        (lib.mkIf (cfg.enableDefaultConfig && cfg.package ? src && lib.isStringLike cfg.package.src) (
          lib.mkOrder 501 defaultConfig
        ))
        (lib.mkIf (settings != "") (lib.mkOrder 999 settings))
      ];

    xdg.configFile."niri/config.kdl" = lib.mkIf (cfg.extraConfig != "") {
