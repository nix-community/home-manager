...
in
{
  config = lib.mkIf cfg.enable {
    systemd.user.services.imapgoose = {
      ...
      // lib.optionalAttrs (cfg.preExec != "") {
        ExecStartPre = [
          (pkgs.writeShellScript "imapgoose-pre-exec" cfg.preExec)
        ];
      }
      // lib.optionalAttrs (cfg.postExec != "") {
        ExecStartPost = pkgs.writeShellScript "imapgoose-post-exec" cfg.postExec;
      };
    };
