  ...
  createQuadletSource = name: volumeDef:
    let
      volumeConfig = (podman-lib.deepMerge {
        Install = {
          WantedBy = (if volumeDef.autoStart then [
            "default.target"
            "multi-user.target"
          ] else
            [ ]);
        ...
        Unit = {
          Description = (if (builtins.isString volumeDef.description) then
            volumeDef.description
          else
...
in {
  ...
  in mkIf cfg.enable {
    ...
    home.file."${config.xdg.configHome}/podman/volumes.manifest".text =
