    ...
    curl() {
        ...
        while
            ! ${pkgs.libxml2}/bin/xmllint \
                ...
                ${config.home.homeDirectory}/.local/state/syncthing/config.xml \
          ...
          Service = {
            ExecStartPre = mkIf (cfg.cert != null || cfg.key != null) "+${
                pkgs.writers.writeBash "syncthing-copy-keys" ''
                  ${install} -dm700 ${config.home.homeDirectory}/.local/state/syncthing/
                  ${optionalString (cfg.cert != null) ''
                    ${install} -Dm400 ${
                      toString cfg.cert
                    } ${config.home.homeDirectory}/.local/state/syncthing/cert.pem
                  ''}
                  ${optionalString (cfg.key != null) ''
                    ${install} -Dm400 ${
                      toString cfg.key
                    } ${config.home.homeDirectory}/.local/state/syncthing/key.pem
                  ''}
                ''
              }";
