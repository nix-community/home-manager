...
in
{
  ...
  imports = [
    (lib.mkChangedOptionModule [ "programs" "git" "patdiff" "enable" ] [ "programs" "patdiff" "enable" ]
      (
        config:
        builtins.trace
          "programs.git.patdiff.enable has been moved to programs.patdiff.enable and programs.patdiff.enableGitIntegration. Please update your configuration."
          (lib.getAttrFromPath [ "programs" "git" "patdiff" "enable" ] config)
      )
    )
  ...
  config =
    let
      oldOptionValue = lib.attrByPath [ "programs" "git" "patdiff" "enable" ] false config;
