...
in
{
  ...
  imports = [
    (lib.mkChangedOptionModule [ "programs" "git" "delta" "enable" ] [ "programs" "delta" "enable" ] (
      config:
      builtins.trace
        "programs.git.delta.enable has been moved to programs.delta.enable and programs.delta.enableGitIntegration. Please update your configuration."
        (lib.getAttrFromPath [ "programs" "git" "delta" "enable" ] config)
    ))
  ...
  config =
    let
      oldOptionValue = lib.attrByPath [ "programs" "git" "delta" "enable" ] false config;
