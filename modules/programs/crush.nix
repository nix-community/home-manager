...
let
  ...
  # Filter MCP server configs (remove transport-specific fields, remove default disabled)
  filterMcpServer =
    server:
    let
  ...
  # Generate secret loading code for wrapper script
...
in
{
  ...
  options.programs.crush = {
    ...
    finalPackage = lib.mkOption {
      ...
      internal = true;
      description = ''
        Wrapped crush package with secret environment variables injected at runtime.
    ...
    enableMcpIntegration = lib.mkOption {
      ...
      default = false;
      description = ''
        If enabled, MCP servers from {option}`programs.mcp.servers` will be merged into {option}`programs.crush.settings.mcp`.

        MCP servers from {option}`programs.mcp.servers` are transformed to Crush format:
    ...
    settings = lib.mkOption {
      ...
      description = ''
        ...
        This will be written to {file}`$HOME/.config/crush/crush.json`.

        Supports configuring LSP servers ({option}`settings.lsp`),
        MCP servers ({option}`settings.mcp`), and AI providers ({option}`settings.providers`).

        For sensitive values like API keys, use {option}`secretEnvVars`
        to inject them from files at runtime.

        See <link xlink:href="https://github.com/charmbracelet/crush"/> for full configuration options.
      '';
    };
    ...
    skillsDir = lib.mkOption {
      ...
      description = ''
        ...
        Skill files from this directory will be symlinked to ~/.config/crush/skills/.
      '';
