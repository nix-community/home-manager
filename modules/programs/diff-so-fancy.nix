...
in
{
  ...
  imports = [
    (lib.mkChangedOptionModule
      ...
      (
        config:
        builtins.trace
          "programs.git.diff-so-fancy.enable has been moved to programs.diff-so-fancy.enable and programs.diff-so-fancy.enableGitIntegration. Please update your configuration."
          (lib.getAttrFromPath [ "programs" "git" "diff-so-fancy" "enable" ] config)
      )
    )
  ...
  config =
    let
      oldOptionValue = lib.attrByPath [ "programs" "git" "diff-so-fancy" "enable" ] false config;
