{ config, lib, pkgs, ... }:
let
  inherit (lib) catAttrs filter mkOption optionalAttrs types;
  ...
  notmuchIni = lib.recursiveUpdate {
    ...
    in {
      ...
      other_email = map (email: email.address or email) (lib.flatten
  ...
  config = lib.mkIf cfg.enable {
    ...
    in {
      "notmuch/default/config".text =
        let toIni = lib.generators.toINI { mkKeyValue = mkIniKeyValue; };
        in ''
