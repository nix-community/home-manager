...
let
  ...
  extensionsDir = runCommand "gram-extensions" { } ''
    set -euo pipefail
...

{
  ...
  config = mkIf cfg.enable {
    ...
    home.packages = optional (cfg.package != null) (
      if cfg.extraPackages == [ ] && cfg.extensionPackages == [ ] then
        cfg.package
      else
        (symlinkJoin {
          ...
          postBuild =
            let
              optionalArgs =
                (optional (cfg.extraPackages != [ ]) "--suffix PATH : ${lib.makeBinPath cfg.extraPackages}")
                ++ (optional (cfg.extensionPackages != [ ]) "--set GRAM_SYSTEM_EXTENSIONS_DIR ${extensionsDir}");
            in
            ''
              wrapProgram $out/bin/${cfg.package.meta.mainProgram or "gram"} \
                ${concatStringsSep " " optionalArgs}
            '';
        })
    );

    home.sessionVariables = mkIf (cfg.package == null && cfg.extensionPackages != [ ]) {
      GRAM_SYSTEM_EXTENSIONS_DIR = extensionsDir;
    };
    ...
    xdg.configFile =
      let
        ...
        singleFileConfigs = filterAttrs (_: value: value != null) {
