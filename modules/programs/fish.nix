  ...
  config = mkIf cfg.enable (mkMerge [
    {
      ...
      programs.fish.interactiveShellInit = ''
        ...
        begin
          ...
          set -l prev_joined (string replace --regex "[^\s]*home-manager-path/share/fish/vendor_completions\.d" "" $joined)
          set -l prev_joined (string replace --regex "[^\s]*generated_completions.*" "" $prev_joined)
          set -l post_joined (string replace $prev_joined "" $joined)
          set -l prev (string split " " (string trim $prev_joined))
          set -l post (string split " " (string trim $post_joined))
          set fish_complete_path $prev "${config.home.path}/share/fish/vendor_completions.d" "${config.xdg.dataHome}/fish/home-manager_generated_completions" $post
        end
      '';
