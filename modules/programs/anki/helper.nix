...
in
{
  ...
  homeManagerAnkiAddon = pkgs.anki-utils.buildAnkiAddon {
    ...
    src = pkgs.writeTextDir "__init__.py" ''
      ...
      # Show a warning when the user clicks "Add" in the profile manager.
      def warningAddProfile(self: AnkiQt) -> None:
        QMessageBox.warning(
          aqt.mw,
          "NixOS Info",
        ...
        return None
      AnkiQt.onAddProfile = warningAddProfile

      # Show a warning when the user clicks "Rename" in the profile manager.
      def warningRenameProfile(self: AnkiQt) -> None:
        QMessageBox.warning(
          aqt.mw,
          "NixOS Info",
          ("Profiles cannot be renamed while settings are managed with "
            "Home Manager.")
        )
        return None
      AnkiQt.onRenameProfile = warningRenameProfile

      # Show a warning when the user clicks "Delete" in the profile manager.
      def warningRemProfile(self: AnkiQt) -> None:
        QMessageBox.warning(
          aqt.mw,
          "NixOS Info",
          ("Profiles cannot be deleted while settings are managed with "
            "Home Manager.")
        )
        return None
      AnkiQt.onRemProfile = warningRemProfile
