...
in
{
  ...
  imports = [
    (lib.mkChangedOptionModule
      ...
      (
        config:
        builtins.trace
          "programs.git.diff-highlight.enable has been moved to programs.diff-highlight.enable and programs.diff-highlight.enableGitIntegration. Please update your configuration."
          (lib.getAttrFromPath [ "programs" "git" "diff-highlight" "enable" ] config)
      )
    )
  ...
  config =
    let
      oldOptionValue = lib.attrByPath [ "programs" "git" "diff-highlight" "enable" ] false config;
