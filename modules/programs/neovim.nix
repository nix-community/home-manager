...
let
  ...
  pluginWithConfigType = types.submodule {
    options = {
      ...
      type = mkOption {
        type = types.str;
        description = "Language used in config. For instance 'viml', 'lua', 'fennel', 'teal', ...";
        default = "viml";
      };
...
in {
  ...
  config = let
    ...
    pluginsNormalized = map (x: if (x ? plugin) then x else { type = x.type or "viml"; plugin = x; config = ""; optional = false;}) cfg.plugins;
    suppressNotVimlConfig = p: if p.type != "viml" then p // { config = ""; } else p;
  ...
  in mkIf cfg.enable {
    ...
    programs.neovim.generatedConfigs = let
      ...
      concatConfigs = lib.concatMapStrings (p: builtins.trace p.plugin.name p.config);
