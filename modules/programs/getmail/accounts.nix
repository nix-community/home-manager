...
in
{
  ...
  config = lib.mkIf config.getmail.enable {
    getmail.settings = {
      retriever = {
        type = lib.mkDefault (
        ...
        password_command = lib.mkDefault "(${
          lib.concatMapStringsSep ", " (x: "'${x}'") config.passwordCommand
        })";
      ...
      options.delete = lib.mkOptionDefault false;
      options.read_all = lib.mkOptionDefault true;
    };
  };
}
