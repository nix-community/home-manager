...
in {
  options.pimsync = {
    ...
    url = mkOption {
      type = dynamicType;
      default = null;
      example = [ "cmd" "secret-store" "--get-caldav-url" ];
    ...
    userName = mkOption {
      type = dynamicType;
      default = null;
      example = [ "cmd" "secret-store" "--get-caldav-username" ];
      description = ""; # TODO: Description
    };
    ...
    password = mkOption {
      type = dynamicType;
      default = null;
      example = [ "shell" "pass" "dav" "|" "head" "-1" ];
    };

    collections = mkOption {
      type = types.oneOf [
        (types.enum [ "all" "fromLocal" "fromRemote" ])
    ...
    conflictResolution = mkOption {
      type = types.nullOr (types.oneOf [
        (types.enum [ "keepLocal" "keepRemote" ])
        (types.listOf types.str)
      ]);
      default = null;
      example = [ "cmd" "nvim" "-d" ];
    ...
    onEmpty = mkOption {
      type = types.nullOr (types.enum [ "skip" "sync" ]);
      default = null;
      description = ""; # TODO: Description
    };
