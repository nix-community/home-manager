  ...
  options.home-manager = {
    enableLegacyProfileManagement = mkOption {
      ...
      defaultText = lib.literalMD ''
        - `true` for `system.stateVersion` < 25.05,
        - `false` otherwise'';
      description = ''
      ...
      home-manager = {
        ...
        sharedModules = [{
          ...
          config = {
            ...
            # Legacy profile management is when the activation script generates
            # GC root and home-manager profile. The modern way simply relies on
            # the GC root that the system maintains, which should also protect
            # the Home Manager activation package outputs.
