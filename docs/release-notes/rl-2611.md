...
- {file}`hm-session-vars.sh` is now safe to re-source, so new Bash, Zsh, and
  Fish shells pick up values changed by `home-manager switch`. Plain session
  variables must not reference themselves; use [](#opt-home.sessionPath),
  [](#opt-home.sessionSearchVariables), or the new
  [](#opt-home.sessionSearchVariablesAppend) instead. Search variables add
  missing entries without moving existing ones; empty entries are ignored, so
  spell out `.` if you want the current directory on a search path. Nested
  Bash shells preserve values owned by Bash login configuration; pre-manifest
  sessions and Bash versions before 4.2 do so conservatively until the next
  login. Interactive
  Bash shells started inside a Fish or Zsh login session have no Bash ownership
  manifest, so they also refresh conservatively. Removing a value still
  requires an environment reset. On Darwin, disable
  {option}`targets.darwin.terminfo.enable` to manage {env}`TERMINFO_DIRS`
  yourself.
