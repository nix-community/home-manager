...
- Mergiraf's [](#opt-programs.mergiraf.enableGitIntegration) and
  [](#opt-programs.mergiraf.enableJujutsuIntegration) options were added to 
  allow explicit control for behavior that was previously automatic. They are
  disabled by default after this release's `stateVersion` and require enablement
  manually.
