...
let
  ...
  forkConfig = forkInputs // {
    profiles = {
      default = {
        ...
        mcp = {
          # servers or mcpServers configuration
          ${mcpServersKey} = {
            everything = {
              enabled = false;
              command = "custom-npx";
  ...
  mcpIntegrationServersOverrideJsonPath = builtins.toFile "${forkInputs.package.pname}-profile-with-override-servers.json.test" ''
    {
      "${mcpServersKey}": {
        "CustomServer": {
          "headers": {},
  ...
  // {
    nmt.script = ''
      if [[ -n "${toString isMutableProfile}" ]]; then
        ...
        assertFileContent "home-files/${mcpDirectory}/.immutable-mcp.json" ${mcpIntegrationServersOverrideJsonPath}
        ...
        assertFileContent "home-files/${mcpDirectory}/mcp.json" ${mcpIntegrationServersOverrideJsonPath}
