...
let
  storePathSkill = pkgs.writeTextDir "skills/from-store-path/SKILL.md" ''
    ---
    name: from-store-path
    description: Skill exposed via a Nix store path string.
    ---
  '';
in
{
  programs.agent-skills = {
    ...
    skills = {
      ...
      from-store-path = "${storePathSkill}/skills/from-store-path";
      from-inline = ''
        ---
        name: from-inline
  ...
  nmt.script = ''
    ...
    assertFileExists home-files/.agents/skills/from-store-path/SKILL.md
    ...
    assertFileRegex home-files/.agents/skills/from-store-path/SKILL.md \
      'Skill exposed via a Nix store path string.'
    assertFileRegex home-files/.agents/skills/from-inline/SKILL.md \
      'Inline skill defined via shared agent-skills.'

    # Non-directory entries at a bundle's top level must not become skills.
    assertPathNotExists home-files/.agents/skills/README.md
  '';
}
