...
let
  ...
  settingsExpected = jsonFormat.generate "settings-expected" (
    cfg.settings
    // {
      inherit (cfg.settings) listenAddress listenPort;
  ...
  profileExpected = jsonFormat.generate "profile-expected" (
    profilePreconfigValue // cfg.extraConfigs."database/profiles/i2c10.json"
  );
...

{
  services.openlinkhub = {
    ...
    extraConfigs = {
      "database/profiles/i2c10.json" = {
        Active = true;
        Path = "/run/user/1000/OpenLinkHub/database/profiles/i2c10.json";
        Product = "Memory";
        Serial = "i2c10";
      };
      "display.json" = ''
  ...
  nmt.script = ''
    ...
    profilePath=$configDir/database/profiles/i2c10.json
